...
class RK2(MovingCameraScene):
    ...
    def construct(self):
        ...
        title = Text("Euler's Method - A numerical method for solving ODE").to_edge(UP).add_background_rectangle(opacity=0.85)
        diff_eqn = MathTex("\\dfrac{dy}{dx}=", "x^2-y^2").next_to(title, direction=DOWN).shift(LEFT * 4).add_background_rectangle(opacity=0.55)
        init_text = MathTex("y(0)=0").move_to(UL).add_background_rectangle()
        ...
        def get_demo_vect():
            # return field.get_vector(point=[main_dot.get_center()[0], main_dot.get_center()[1]])
            ...
            vec.rotate(
                np.arctan(
                    func(main_dot.get_center()[0], main_dot.get_center()[1])
                ),
            ).shift(main_dot.get_center()).shift(vec.get_vector() / 2)
            return vec
        ...
        self.wait()
        self.play(slope_field.animate.fade(.6))
        # self.play(self.camera.frame.animate.scale(.75))
        # self.wait(.5)
        # self.play(Create(init_text))
        self.play(Create(main_dot), Create(demo_vect))
        # self.wait()
        # self.play(FadeOut(init_text))

        step_size = Text("Step Size =").to_corner(UL).shift(DOWN * .75 + RIGHT)
        ...
        for interval, width, j in zip(intervals, widths, list(range(len(intervals)))):
            ...
            for i in range(interval):
                ...
                self.play(main_dot.animate.move_to(intersection_point))
                if j != 0:
                ...
                self.play(Create(slope_n))

                self.play(pseudo_dot.animate.move_to(pseudo_intersection_point))
                ...
                self.play(Create(pseudo_slope_line))
                ...
                # self.play(Create(plus), run_time=2)
                # self.wait()
